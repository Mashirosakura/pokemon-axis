...
# Custom added
BattleHandlers::SpeedCalcAbility.add(:FEATHEREDDOWN,
  proc { |ability,battler,mult|
    w = battler.battle.pbWeather
    next mult*1 if w==PBWeather::Hail
  }
)
...
#Custom added
BattleHandlers::MoveBaseTypeModifierAbility.add(:TUNDRATUNE,
  proc { |ability,user,move,type|
    next getConst(PBTypes,:ICE) if hasConst?(PBTypes,:ICE) && move.soundMove?
  }
)

#Custom added
BattleHandlers::MoveBaseTypeModifierAbility.add(:CLEARBELL,
  proc { |ability,user,move,type|
    next getConst(PBTypes,:PSYCHIC) if hasConst?(PBTypes,:PSYCHIC) && move.soundMove?
  }
)
...
BattleHandlers::DamageCalcUserAbility.add(:HEADACHE,
  proc { |ability,user,target,move,mults,baseDmg,type|
    if @effects[PBEffects::Confusion]>0 && move.specialMove?
      mults[ATK_MULT] *= 1.5
    end
  }
)
...
#Custom added
BattleHandlers::DamageCalcUserAbility.add(:STRONGLEG,
  proc { |ability,user,target,move,mults,baseDmg,type|
    mults[BASE_DMG_MULT] *= 1.2 if move.kickingMove?
  }
)
...
BattleHandlers::DamageCalcUserAbility.add(:SPELLCASTER,
  proc { |ability,user,target,move,mults,baseDmg,type|
    ...
    mults[ATK_MULT] *= 1.5 if isConst?(type,PBTypes,:WATER)
    mults[ATK_MULT] *= 1.5 if isConst?(type,PBTypes,:FLYING)
    mults[ATK_MULT] *= 1.5 if isConst?(type,PBTypes,:GROUND)
  }
)
...
BattleHandlers::DamageCalcTargetAbility.copy(:FILTER,:SOLIDROCK,:CRYSTALLIZED)
...
# Custom added
BattleHandlers::DamageCalcTargetAbility.add(:FEATHEREDDOWN,
  proc { |ability,user,target,move,mults,baseDmg,type|
    mults[FINAL_DMG_MULT] *= 2 if isConst?(move.calcType,PBTypes,:WATER)
    mults[FINAL_DMG_MULT] *= 2 if isConst?(move.calcType,PBTypes,:ICE)
  }
)
...
BattleHandlers::TargetAbilityOnHit.copy(:IRONBARBS,:ROUGHSKIN,:PUFFUP)
...
BattleHandlers::AbilityOnSwitchIn.add(:TOXICSURGE,
  proc { |ability,battler,battle|
    next if battle.field.terrain==PBBattleTerrains::Toxic
    battle.pbShowAbilitySplash(battler)
    battle.pbStartTerrain(battler,PBBattleTerrains::Toxic)
    # NOTE: The ability splash is hidden again in def pbStartTerrain.
  }
)
