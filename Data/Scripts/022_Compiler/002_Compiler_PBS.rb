...
def pbCompileEasyTrainers
  ...
  oldcompilerline   = 0
  oldcompilerlength = 0
  pbCompilerEachCommentedLine("PBS/trainersE.txt") { |line,lineno|
    if line[/^\s*\[\s*(.+)\s*\]\s*$/]
...
def pbCompileHardTrainers
  ...
  oldcompilerline   = 0
  oldcompilerlength = 0
  pbCompilerEachCommentedLine("PBS/trainersH.txt") { |line,lineno|
    if line[/^\s*\[\s*(.+)\s*\]\s*$/]
      # Section [trainertype,trainername] or [trainertype,trainername,partyid]
      if oldcompilerline>0
        raise _INTL("Previous trainer not defined with as many Pokémon as expected.\r\n{1}",FileLineData.linereport)
