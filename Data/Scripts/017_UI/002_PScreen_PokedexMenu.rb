...
class Window_DexesList < Window_CommandPokemon
  ...
  def drawItem(index,count,rect)
    ...
    if index>=0 && index<@commands2.length
      pbDrawShadowText(self.contents,rect.x+254+RXMOD,rect.y,64,rect.height,
         sprintf("%d",@commands2[index][0]),self.baseColor,self.shadowColor,1)
      pbDrawShadowText(self.contents,rect.x+350+RXMOD,rect.y,64,rect.height,
      ...
      pbDrawImagePositions(self.contents,[
        ["Graphics/Pictures/Pokedex/icon_menuseenown",rect.x+236+RXMOD,rect.y+4,(allseen) ? 24 : 0,0,24,24],
        ["Graphics/Pictures/Pokedex/icon_menuseenown",rect.x+332+RXMOD,rect.y+4,(allown) ? 24 : 0,24,24,24]
      ])
    end
  end
end
...
class PokemonPokedexMenu_Scene
  ...
  def pbStartScene(commands,commands2)
    ...
    @sprites["headings"]=Window_AdvancedTextPokemon.newWithSize(
       _INTL("<c3=F8F8F8,C02028>SEEN<r>OBTAINED</c3>"),286+RXMOD,136,208,64,@viewport)
